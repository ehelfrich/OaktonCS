...
// Edited by Eric Helfrich for LargeInt multiplication
// 
...
public class LargeInt {
	...
	private static SpecializedList multiplyLists(SpecializedList multiplicand, byte multiplier) {
		//Multiplies a one digit multiplier and a multiplicand
		byte digitMulti;
		byte digitTop;
		byte temp;
		byte carry = 0;

		int multiplicandLength = multiplicand.size();

		SpecializedList result = new SpecializedList();

		multiplicand.resetBackward();

		digitMulti = multiplier;

		for (int i = 1; i <= multiplicandLength; ++i) {
			digitTop = multiplicand.getPriorElement();
			temp = (byte) ((digitMulti * digitTop) + carry);
			carry = (byte) (temp / 10);
			result.addFront((byte) (temp % 10));
		}
		if (carry != 0) {
	...
	public static LargeInt multiply(LargeInt first, LargeInt second) { //first = top and second = bottom
		// Returns a LargeInt that is the product of the two argument LargeInts
		LargeInt product = new LargeInt();
		LargeInt temp = new LargeInt();
		byte multiplier;

		if (first.sign == second.sign) { //signs are the same
			second.numbers.resetBackward();
			for (int i = 0; i < second.numbers.size(); ++i) {
				multiplier = second.numbers.getPriorElement();
				temp.numbers = multiplyLists(first.numbers, multiplier);

				for (int j = 0; j < i; ++j) { //adds zeros to end of temp.numbers for multipliers > 9
					temp.numbers.addEnd((byte) 0);
				}

				if (i == 0) {
					product.numbers = temp.numbers;
				}
				else {
